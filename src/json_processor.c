...
{
    json_state.current_health.cpu        = 23.4f;
    json_state.current_health.memory     = 58.7f;
    json_state.current_health.disk       = 72.1f;
    json_state.current_health.net_in     = 102.5f;
    json_state.current_health.net_out    = 88.3f;
    json_state.current_health.processes  = 47;
    json_state.current_health.valid      = true;

    json_state.sample_count = 1;
    json_state.last_data_time = to_ms_since_boot(get_absolute_time());
    json_state.is_connected = true;
    ...
    printf("   NET ↓: %.1f KB/s, ↑: %.1f KB/s, PROC: %d\n", 
           json_state.current_health.net_in, 
           json_state.current_health.net_out, 
           json_state.current_health.processes);
}
...
void json_processor_reset(void)
{
    ...
    printf("JSON Processor: State reset\n");
}